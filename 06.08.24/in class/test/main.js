...
// 2. read each line
// 2. if myProducts.categories includes "electronics" ad to an array
// return all arrays
/*
VOLTAR AQUI
const myProducts = [
...
function getAveragePrice(products) {
  ...
  let avaragePrice = sumPrices/quantityProducts
  return avaragePrice
}
...
let s_count = countCharacterOccurrences(strings, "s");
...
function getUppercaseStrings(strings) {
    ...
    let newString = ''
    for (let i = 0; i < strings.length; i++){
        newString = strings[i].getUppercaseStrings
        for (let j = 0; j < strings[i]; j++){
            newArray += newString
        }
    }
  return newArray
